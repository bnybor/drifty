...
| 13 | **[funnel](13_funnel)** | A streaming, detection-routed **adaptive receiver over the full concatenated stack** (`rs251` → `marker` → `cc`): the `detect_clean` / `detect_noisy` detectors gate each block of one time-varying channel to the cheap (`bcjr`) or drift-robust (`maxir`) **soft** decoder — or drop it — escalating only as needed; the outer marker + `rs251` soft layers then reframe and mop up the inner decoder's residual errors. Ties together the pipes, detectors, block/frame codecs, and the soft path end to end. |
| 14 | **[pipeline](14_pipeline)** | Example 13 rebuilt so the **entire funnel is one composed pipe graph** (see `layout.dot`): splitters, two diverters steered by two **executor** pipes, a combiner, and the frame pipes, held in a pipe container. The caller only pushes channel bits and reads recovered frames from the final frame pipe. Split across `main` / `funnel` / `stack` files, with `dt_container` for one-shot cleanup. |
