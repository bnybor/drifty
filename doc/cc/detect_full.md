...
`detect_full` is the **noise-tolerant** of drifty's two blind detectors. Where
[`detect_lean`](detect_lean.md) needs near-exact parity (it holds only to ~1 %
flips), detect_full scores the **bias** of approximate parity checks with a fast
...
matters, prefer the cheaper [`detect_lean`](detect_lean.md); the two share this
...
`(1 − 2p)^w` rather than destroying the check (contrast detect_lean's exact rank,
...
preset, clean-channel model, over a long stream (`detect_full` self-tests and the
...
dt_stream_soft_decoder *dt_cc_detect_full_soft_decoder_create(const dt_cc_detect_full_stream_params *params);
...
[`maxir`](maxir.md)** (and [`detect_lean`](detect_lean.md)), so a channel you
...
- **Noise envelope — graceful but bounded.** Reliable to ~5 % flips (marginal to
  ...
  honest output is "uncertain", and detect_full keeps coded streams measurably above
  random even there.
- **Cost.** One `2^{L_c}`-entry histogram (~64 KB) plus a Walsh–Hadamard transform
  per window-stride — roughly one to two orders more work per bit than detect_lean,
  whose state is a few KB with no transform. On a clean channel detect_lean reaches
  the same verdict for far less, so spend detect_full only where the noise warrants.
...
- [`detect_lean`](detect_lean.md) — the cheap, embeddable sibling (same API and
