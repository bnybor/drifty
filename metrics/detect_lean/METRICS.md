...
`metrics/detect_lean/dt_detect_lean_metrics.c` measures the **detect_lean** blind
code-presence detector ([doc](../../doc/cc/detect_lean.md)) — exact GF(2)
...
detect_lean does not recover bits, so the metrics are detection confidence rather
than edit distance.
...
detect_lean answers "is a convolutional code present?". For each point we run **two**
...
The model only calibrates **`c_absent`**: detect_lean damps its no-code confidence by
...
cmake --build build --target dt_detect_lean_metrics

# Coarse pass (fast - what is committed):
build/metrics/detect_lean/dt_detect_lean_metrics 4 2500 0xC0FFEE pegged  > metrics/detect_lean/untuned/metrics.csv
build/metrics/detect_lean/dt_detect_lean_metrics 4 2500 0xC0FFEE matched > metrics/detect_lean/tuned/metrics.csv

# Full sweep (more trials / bits; run when done iterating):
build/metrics/detect_lean/dt_detect_lean_metrics 40 6000 0xC0FFEE pegged  > metrics/detect_lean/untuned/metrics.csv
build/metrics/detect_lean/dt_detect_lean_metrics 40 6000 0xC0FFEE matched > metrics/detect_lean/tuned/metrics.csv
...
.venv/bin/python metrics/detect_lean/plot_metrics.py metrics/detect_lean/untuned/metrics.csv -o metrics/detect_lean/untuned/plots/
.venv/bin/python metrics/detect_lean/plot_metrics.py metrics/detect_lean/tuned/metrics.csv   -o metrics/detect_lean/tuned/plots/
```
...
detect_lean holds full separation only at low rates (exact parity), decaying past
...
The engine constants worth sweeping live in `src/cc/detect_lean/decode.c`
...
[detect_full](../detect_full/METRICS.md), whose bias method pushes every knee
